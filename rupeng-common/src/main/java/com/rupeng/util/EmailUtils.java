...
public class EmailUtils {
    ...
    public static int checkEmailCode(HttpSession session, String email, String requestCode) {
        ...
        if (!email.equals(sessionEmail)) {
            logger.debug("检查邮件验证码失败，用户填写的邮箱和验证码发送的邮箱不一致，email：{}，requestCode：{}，sessionEmail：{}", email, requestCode, sessionEmail);
            return CHECK_RESULT_FLASE;
        }
    ...
    //发送邮件验证码
    public static void sendEmailCode(HttpSession session, String smtpServer, String username, String password, String from, String to) {
    ...
    //使用sendcloud发送验证码邮件
    public static void sendEmailCodeWithSendCloud(HttpSession session, String url, String apiUser, String apiKey, String from, String to, String templateInvokeName) {
